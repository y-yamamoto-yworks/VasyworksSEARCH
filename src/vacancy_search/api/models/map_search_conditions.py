...
class MapSearchConditions(object):
    """検索条件"""
    def __init__(self, *args, **kwargs):
        ...
        self.layout_categories = None  # 間取種別カテゴリ
        self.room_area_lower = None     # 専有面積下限
        self.room_area_upper = None     # 専有面積上限
    ...
    def set_conditions(self, url_params):
        ...
        self.layout_categories = UrlParamHelper.get_id_array_param('lay', url_params)       # 間取り種別カテゴリID
        self.room_area_lower = UrlParamHelper.get_int_param('l_ra', url_params)     # 専有面積下限
        self.room_area_upper = UrlParamHelper.get_int_param('u_ra', url_params)      # 専有面積上限
    ...
    FROM句用SQL
    """
    def get_only_own_property_sql(self, is_building):
        ans = None
    ...
    def get_layout_categories_sql(self, params):
        """間取り種別"""
        ans = None
        if self.layout_categories:
            targets = self.__get_id_targets('layout_category', self.layout_categories, params)
            ans = 'layout_type.category_id IN ({0})'.format(targets)
        return ans

    def get_room_area_sql(self, params):
        """専有面積"""
        ans = None
        if xint(self.room_area_lower) > 0:
            ans = 'room.room_area >= %(room_area_lower)s'
            params['room_area_lower'] = self.room_area_lower
        if xint(self.room_area_upper) > 0:
            ans = 'room.room_area <= %(room_area_upper)s'
            params['room_area_upper'] = self.room_area_upper
        return ans
