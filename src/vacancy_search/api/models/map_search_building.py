...
class MapSearchBuilding(models.Model):
    """建物リスト"""
    ...
    layout = models.CharField(_('layout'), db_column='layout', max_length=255, null=True, blank=True)
    rent = models.CharField(_('rent'), db_column='rent', max_length=255, null=True, blank=True)
    room_area = models.CharField(_('room_area'), db_column='room_area', max_length=255, null=True, blank=True)
    ...
    """
    @classmethod
    def get_buildings(cls, conditions: MapSearchConditions, is_residential):
        """クエリセットの取得"""
        params = {}
        ...
        for bldg in ans:
            ...
            lower_room_area = 0
            upper_room_area = 0
            for bldg_room in rooms:
                if bldg_room.layout_type_text not in layouts:
                    if bldg_room.layout_type_text:
                        layouts.append(bldg_room.layout_type_text)

                if lower_rent == 0 or lower_rent > bldg_room.rent:
                    lower_rent = bldg_room.rent

                if upper_rent == 0 or upper_rent < bldg_room.rent:
                    upper_rent = bldg_room.rent

                if upper_rent < bldg_room.rent_upper:
                    upper_rent = bldg_room.rent_upper

                if lower_room_area == 0 or lower_room_area > bldg_room.room_area:
                    lower_room_area = bldg_room.room_area

                if upper_room_area == 0 or upper_room_area < bldg_room.room_area:
                    upper_room_area = bldg_room.room_area

            if len(layouts) > 0:
                bldg.layout = ','.join(layouts)

            if lower_rent > 0:
                bldg.rent = '{:,} 円'.format(lower_rent)
                if upper_rent > lower_rent:
                    bldg.rent += '～{:,} 円'.format(upper_rent)

            if lower_room_area > 0:
                bldg.room_area = '{0} m2'.format(lower_room_area)
                if upper_room_area > lower_room_area:
                    bldg.room_area += '～{0} m2'.format(upper_room_area)
    ...
    def __get_sql_columns(cls):
        ...
        ans += ', null AS layout'
        ans += ', null AS rent'
        ans += ', null AS room_area'

        return ans

    @classmethod
    def __get_sql_tables(cls, conditions: MapSearchConditions, params, is_residential):
        """SQLのテーブル一覧"""
        ans = 'building'
        ...
        if conditions:
            # 管理種別の限定有無
            ans += xstr(conditions.get_only_own_property_sql(is_building=True))
        ...
        ans += 'SELECT room.building_id FROM {0} WHERE {1} GROUP BY room.building_id'.format(
            ...
            cls.__get_sql_room_conditions(conditions=conditions, params=params, is_residential=is_residential),
        )
    ...
    @classmethod
    def __get_sql_room_tables(cls, conditions: MapSearchConditions, params, is_residential):
        """SQLの部屋テーブル一覧"""
        ans = 'room'
        ...
        ans += ' INNER JOIN layout_type ON room.layout_type_id = layout_type.id'
        ans += ' INNER JOIN pet_type on room.pet_type_id = pet_type.id'

        if conditions:
            # 管理種別の限定有無
            ans += xstr(conditions.get_only_own_property_sql(is_building=False))

        return ans

    @classmethod
    def __get_sql_conditions(cls, conditions: MapSearchConditions, params):
        """SQLの建物用条件一覧"""
    ...
    @classmethod
    def __get_sql_room_conditions(cls, conditions: MapSearchConditions, params, is_residential):
        """SQLの部屋用条件一覧"""
        ans = 'room.is_deleted = FALSE AND building.is_deleted = FALSE'
        if not is_residential:
            # 店舗付住宅、住宅付店舗も非居住用賃貸の検索対象に加える
            ans += ' AND (rental_type.is_non_residential = TRUE OR building.building_type_id IN (50, 51))'
        ...
        if conditions:
            ...
            ans += cls.__and_condition_sql(conditions.get_layout_categories_sql(params=params), ans)
            ans += cls.__and_condition_sql(conditions.get_room_area_sql(params=params), ans)
    ...
    @classmethod
    def __get_sql_orders(cls, conditions: MapSearchConditions, params):
        """SQLの並び順一覧"""
    ...
    def exterior_picture(self):
        return BuildingExteriorPicture.get_picture(self.id)
