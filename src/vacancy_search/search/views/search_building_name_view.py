...
class SearchBuildingNameView(FormView):
    """
    建物名称検索
    """
    form_class = SearchBuildingNameForm
    template_name = 'search/search_building_name.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        context['searched_building_name'] = self.building_name

        buildings = None
        if self.building_name:
            conditions = Q(is_deleted=False)
            conditions.add(Q(
                Q(building_name__contains=self.building_name)
                | Q(building_old_name__contains=self.building_name)
            ), Q.AND)
